...
def print_table(games: list[Game], sort_by: str, show_tags: bool = False) -> None:
    header = (
        f"{'#':>3}  {t('report.col_name'):<45}  {t('report.col_year'):>4}  {'MC':>4}  "
        f"{'Steam':>6}  {'HLTB':>5}  {t('report.col_played'):>8}  {t('report.col_score'):>8}"
    )
    print(header)
    print("-" * len(header))
...
def list_collections_cmd(collection_map: dict[str, list[str]]) -> None:
    ...
    if not counter:
        print(t("report.no_collections"))
        return
    print(f"\n{'─' * 40}")
    print(t("report.available_collections", count=len(counter)))
    print(f"{'─' * 40}")
    ...
    for entry in cache.values():
        ...
        for v in values:
            if v.lower() not in STEAM_NOISE_CATEGORIES:
                counter[v] += 1
    if not counter:
        print(t("report.no_items", field=field))
        return
    print(f"\n{'─' * 40}")
    print(t("report.available_items", field=field.upper(), count=len(counter)))
    print(f"{'─' * 40}")
...
def save_results(games: list[Game], output_base: str) -> None:
    ...
    with md_path.open("w", encoding="utf-8") as f:
        f.write(
            f"| # | {t('report.col_name')} | MC | Steam | HLTB | "
            f"{t('report.col_played')} | {t('report.col_score')} |\n"
        )
        f.write("|---|------|----|-------|------|---------|-------|\n")
        for i, g in enumerate(games, 1):
            mc = str(g["metacritic"]) if g["metacritic"] else "-"
    ...
    print(t("report.saved", csv=csv_path, md=md_path))
